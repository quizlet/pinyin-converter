...
window.PinyinConverter =
    ...
    getReplacement: (match) ->
        ...
        for base, vowel of accentMap
            vowelNum = @vowels[vowel]
            replacement = @pinyin[tone.toString()][vowelNum]
            replacedWord = word.replace(base, replacement)
            return replacedWord unless replacedWord is word
        match
